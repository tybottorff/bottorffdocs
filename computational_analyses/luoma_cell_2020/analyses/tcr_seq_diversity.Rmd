...
cdr3_amino_acid_diversity_df <- rna_tcr %>%
  filter(!(is.na(cdr3)),
         Chain != "Multi",
         group != "no ICI") %>%
  group_by(cdr3, v_gene, j_gene, Chain, orig.ident, group, predicted.celltype.l2) %>%
...
downsampled_cdr3_amino_acid_diversity_df$group <- factor(downsampled_cdr3_amino_acid_diversity_df$group, levels = c("no ICI", "ICI no colitis", "ICI colitis"))

p <- ggplot(downsampled_cdr3_amino_acid_diversity_df %>% filter(predicted.celltype.l2 %in% c("CD4 Proliferating", "CD8 Naive", "CD8 TEM", "Treg")), aes(x = Chain, y = shannon, fill = group)) +
  geom_boxplot() +
  facet_wrap(~ predicted.celltype.l2, scales = "free_y") +
  labs(x = "Chain",
       y = "Shannon entropy",
...
cdr3_amino_acid_diversity_df <- rna_tcr %>%
  ...
  slice(1) %>%
  ungroup() %>%
  select(Chain, group, orig.ident, gini, predicted.celltype.l2)
  #group_by(Chain, predicted.celltype.l2) %>%
  #summarize(
  #    p_val_diversity = {
  #      if(length(unique(group)) != 2 | any(table(group) == 1)) {
...
p <- ggplot(cdr3_amino_acid_diversity_df %>% filter(predicted.celltype.l2 %in% c("CD4 Proliferating", "CD8 Naive", "CD8 TEM", "Treg")), aes(x = Chain, y = gini, fill = group)) +
  geom_boxplot() +
  facet_wrap(~ predicted.celltype.l2, scales = "free_y") +
  labs(x = "Chain",
       y = "Shannon entropy",
