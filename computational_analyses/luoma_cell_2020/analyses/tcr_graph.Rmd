...
tcrs <- read.csv("../saved_data/tcr_graph_input.csv") %>%
  ...
  rowwise() %>%
  mutate(aa = list(translate(DNAString(nt)))) %>%
...
# Make TCR graph with Igraph: no irAE (nt link)
...
# Make TCR graph with Igraph: no irAE (aa link)
...
no_irae_tcr_graph <- makeTcrGraph(no_irae_tcrs, link = "aa")
...
# Make TCR graph with Igraph: yes irAE (nt link)
...
# Make TCR graph with Igraph: yes irAE (aa link)
...
irae_tcr_graph <- makeTcrGraph(irae_tcrs, link = "aa")
