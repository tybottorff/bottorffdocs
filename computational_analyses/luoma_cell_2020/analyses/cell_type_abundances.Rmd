...
cd4_cell_abundance_df$group <- factor(cd4_cell_abundance_df$group, levels = c("no ICI", "ICI no colitis", "ICI colitis"))

write.csv(cd4_cell_abundance_df, "../saved_data/cd4_abundances.csv")

ggplot(cd4_cell_abundance_df %>% filter(predicted.celltype.l2 %in% c("Treg", "CD4 TEM", "CD4 TCM", "CD4 Trm", "CD4 Proliferating")), aes(x = predicted.celltype.l2, y = log10(normalized_count), fill = group)) +
  geom_boxplot() +
  labs(x = "", fill = "group", y = "CD4 T cell\nproportion (log10)") +
  scale_y_continuous(expand = expansion(mult = c(0.1, 0.5)), breaks = pretty_breaks(n = 3)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10))

#ggplot(cd4_cell_abundance_df, aes(x = patient, y = log10(normalized_count), color = group)) +
#  geom_point() +
...
cd8_cell_abundance_df$group <- factor(cd8_cell_abundance_df$group, levels = c("no ICI", "ICI no colitis", "ICI colitis"))

ggplot(cd8_cell_abundance_df %>% filter(predicted.celltype.l2 %in% c("CD8 TCM", "CD8 Trm", "CD8 Naive", "CD8 Proliferating")), aes(x = predicted.celltype.l2, y = log10(normalized_count), fill = group)) +
  geom_boxplot() +
  labs(x = "", fill = "group", y = "CD8 T cell\nproportion (log10)") +
  scale_y_continuous(expand = expansion(mult = c(0.1, 0.5)), breaks = pretty_breaks(n = 3)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10))

#ggplot(cd8_cell_abundance_df, aes(x = patient, y = log10(normalized_count), color = group)) +
#  geom_point() +
#  facet_wrap(~predicted.celltype.l2, ncol = 2) +
#  labs(x = "", fill = "group", y = "CD8 T cell\nproportion (log10)") +
#  scale_y_continuous(expand = expansion(mult = c(0.1, 0.5)), breaks = pretty_breaks(n = 3)) +
