...
tigit_klrg1_counts_and_group_data <- group_data %>%
  ...
  group_by(group, orig.ident, TIGIT_status, KLRG1_status) %>%
  summarize(count = n()) %>%
  filter(group != "no ICI",
         TIGIT_status == "pos",
         KLRG1_status == "pos")
...
wilcox.test(tigit_klrg1_counts_and_group_data$count ~ tigit_klrg1_counts_and_group_data$group)

ggplot(tigit_klrg1_counts_and_group_data, aes(x = group, y = count, color = group)) +
  geom_boxplot() + 
  labs(x = "irAE status", y = "TIGIT+ KLRG1+\nnon-naive CD8 count", color = "irAE status")
