...
test <- tcr %>%
  ...
  dplyr::filter(predicted.celltype.l2 == "CD8 Trm") %>%
  dplyr::filter(!(is.na(pgen_TRA)))
...
# Check overall levels of proliferation/activation markers in irAE groups

```{r}

VlnPlot(tcr_and_rna, features = c("MKI67", "CD40LG", "CD38", "HLA-DRB1", "HLA-DRA"), group.by = "group", pt.size = 0)

```

# DEGs for irAE groups vs. each other (no vs. yes irAE)

```{r}

tcr_and_rna_ici <- subset(tcr_and_rna, subset = group != "no ICI")

tcr_and_rna_ici_pseudobulk <- AggregateExpression(tcr_and_rna_ici, assays = "SCT", return.seurat = T, group.by = c("orig.ident", "group"))

tcr_and_rna_ici_pseudobulk$group <- factor(tcr_and_rna_ici_pseudobulk$group, levels = c("ICI colitis", "ICI no colitis"))

Idents(tcr_and_rna_ici_pseudobulk) <- "group"

#Idents(cd4_prolif_ici_pseudobulk)

tcr_and_rna_ici_pseudobulk_degs <- FindMarkers(object = tcr_and_rna_ici_pseudobulk, 
                         ident.1 = "ICI colitis", 
                         ident.2 = "ICI no colitis")

tcr_and_rna_ici_pseudobulk_degs
...
non_naive_cd8s <- subset(tcr_and_rna, subset = predicted.celltype.l1 == "CD8 T" & predicted.celltype.l2 %in% c("CD8 TEM", "CD8 TCM", "CD8 Proliferating"))

VlnPlot(non_naive_cd8s, features = c("TIGIT", "KLRG1", "EOMES"), group.by = "group", pt.size = 0)

```

# Look for TIGIT+KLRG1+ counts

```{r}

group_data <- non_naive_cd8s@meta.data %>% dplyr::select(group, orig.ident)

group_data$barcode <- rownames(group_data)

rownames(group_data) <- NULL

tigit_klrg1_counts <- FetchData(non_naive_cd8s, vars = c("TIGIT", "KLRG1"))

tigit_klrg1_counts$barcode <- rownames(tigit_klrg1_counts)

rownames(tigit_klrg1_counts) <- NULL

tigit_klrg1_counts_and_group_data <- group_data %>%
  left_join(tigit_klrg1_counts, by = "barcode") %>%
  dplyr::filter(group != "no ICI") %>%
  mutate(TIGIT_status = if_else(TIGIT > 0, "pos", "neg"),
         KLRG1_status = if_else(KLRG1 > 0, "pos", "neg")) %>%
  group_by(group, orig.ident) %>%
  summarize(total_count = n(),
            double_pos_count = sum(TIGIT_status == "pos" & KLRG1_status == "pos")) %>%
  mutate(freq = double_pos_count / total_count) %>%
  dplyr::select(group, orig.ident, freq)

tigit_klrg1_counts_and_group_data

wilcox.test(tigit_klrg1_counts_and_group_data$freq ~ tigit_klrg1_counts_and_group_data$group)

ggplot(tigit_klrg1_counts_and_group_data, aes(x = group, y = freq, color = group)) +
  geom_boxplot() + 
  labs(x = "irAE status", y = "TIGIT+ KLRG1+\nnon-naive CD8 frequency", color = "irAE status")

```

# Export Anndata object for mvTCR
...
just_T_cells <- AddMetaData(just_T_cells, metadata = metadata)

#just_T_cells_normalized <- NormalizeData(just_T_cells)

just_T_cells[["RNA"]] <- as(just_T_cells[["RNA"]], "Assay")

sceasy::convertFormat(just_T_cells, from="seurat", to="anndata", outFile='../saved_data/t_cells.h5ad')

```

# Export clean TCR csv for mvTCR

```{r}

tcrs <- read.csv("../saved_data/tcrs.csv") %>%
...
tcrs_cleaned <- tcrs[-c(99571, 94594), ]

write.csv(tcrs_cleaned, "../saved_data/tcrs_for_mvtcr.csv")
