...
# Read in IGoR output to use for TCR graph

```{r}

igor_alpha <- read.csv("../saved_data/igor_output_alpha.csv") %>%
  dplyr::rename(barcode = TCR_name) %>%
  left_join(tcrs, by = "barcode") %>%
  select(barcode, patient, Chain, v_gene, j_gene, TRA_nt, TRA_CDR3) %>%
  dplyr::rename(nt = TRA_nt,
                CDR3 = TRA_CDR3) %>%
  filter(Chain == "TRA")
...
igor_combined <- read.csv("../saved_data/igor_output_beta.csv") %>%
  dplyr::rename(barcode = TCR_name) %>%
  left_join(tcrs, by = "barcode") %>%
  select(barcode, patient, Chain, v_gene, j_gene, TRB_nt, TRB_CDR3) %>%
  dplyr::rename(nt = TRB_nt,
                CDR3 = TRB_CDR3) %>%
  filter(Chain == "TRB") %>%
  full_join(igor_alpha, by = c("barcode", "Chain", "patient", "v_gene", "j_gene", "nt",)) %>%
...
write.csv(igor_combined, "../saved_data/tcr_graph_input.csv")
