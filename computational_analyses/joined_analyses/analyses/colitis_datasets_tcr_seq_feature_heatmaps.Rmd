...
# Perform feature tests between irAE groups within each cell type, chain combo

```{r}

combined_analysis_data <- prepped_data %>%
  mutate(dataset = "Combined") %>%
  full_join(prepped_data)

all_tcrs <- combined_analysis_data %>%
  group_by(predicted.celltype.l2, Chain, dataset) %>%
  summarize(
      p_val_pgen = {
        if(length(unique(group)) != 2 | any(table(group) == 1)) {
          NA
  ...
  filter(p_val < 0.1) %>%
  arrange(predicted.celltype.l2, Chain, Feature)

all_tcrs

# CD8 Trm (both chains pgen and TRB cdr3 length) and CD8 proliferating TRB pgen features different between irAE groups

prepped_data %>%
  filter(predicted.celltype.l2 %in% c("CD4 Proliferating", "CD8 Trm")) %>%
  group_by(dataset, predicted.celltype.l2) %>%
  summarize(count = n()) %>%
  arrange(predicted.celltype.l2)

# there are barely any unique clonotypes for CD4 proliferating T cells in Thomas dataset (~200 vs. Luoma ~1200), so perhaps that's related to not seeing effect there (too small sample size)

combined_plot_data <- combined_analysis_data %>%
  mutate(dataset = case_when(
         dataset == "2020_cell_luoma" ~ "Luoma (2020), Cell",
         dataset == "2021_biorxiv_thomas" ~ "Thomas (2021), bioRxiv",
         dataset == "Combined" ~ "Combined")) %>%
  filter(Chain == "TRB")

combined_plot_data$group <- factor(combined_plot_data$group, levels = c("ICI no colitis", "ICI colitis"))

plot <- ggplot(combined_plot_data %>% filter(predicted.celltype.l2 %in% c("CD8 Trm", "CD4 Proliferating")), aes(x = as.factor(predicted.celltype.l2), y = log10(pgen), fill = group)) +
  geom_boxplot() +
  facet_wrap(~dataset) +
  labs(x = "", y = "pgen (log10)") +
  theme(strip.text = element_text(size = 12),
...
ggsave("../figures/cd8_trm_and_cd4_prolif_features_all_unique_tcrs.svg", plot, width = 10, height = 5)

# CD8 Trm TRB CDR3 length and pgen seem best bets across both datasets & combined, also worth showing CD4 proliferating TRB pgen as that was main luoma result (don't see in Thomas/combined)
