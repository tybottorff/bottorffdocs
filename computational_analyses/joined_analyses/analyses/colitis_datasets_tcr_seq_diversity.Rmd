...
# Analyze diversity via Shannon entropy
 ...
 - see differences for CD4 prolif TRA/TRB, CD8 naive TRA/TRB, CD8 proliferating TRA/TRB, don't see when group sizes are equalized though...

```{r}

cdr3_amino_acid_diversity_df <- combined_data %>%
  filter(!(is.na(cdr3)),
         Chain != "Multi",
         group != "no ICI") %>%
  group_by(cdr3, v_gene, j_gene, Chain, patient, group, predicted.celltype.l2) %>%
  mutate(cdr3_amino_acid_count = n()) %>%
  ungroup() %>%
  group_by(patient, Chain, predicted.celltype.l2) %>%
  ...
  slice(1) %>%
  ungroup() %>%
  select(Chain, group, patient, shannon, inv_shannon, predicted.celltype.l2) %>%
...
cdr3_amino_acid_diversity_df <- rna_tcr %>%
  ...
  slice(1) %>%
  ungroup() %>%
  select(Chain, group, patient, gini, predicted.celltype.l2) %>%
  group_by(Chain, predicted.celltype.l2) %>%
  summarize(
      p_val_diversity = {
        if(length(unique(group)) != 2 | any(table(group) == 1)) {
