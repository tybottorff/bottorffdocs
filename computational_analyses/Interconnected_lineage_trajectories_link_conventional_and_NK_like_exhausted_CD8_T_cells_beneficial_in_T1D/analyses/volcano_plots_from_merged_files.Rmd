...
cells <- rep(c("CD57pos_DP_NN_CD8", "CD57neg_DP_NN_CD8", "DN_NN_CD8"), times = 1)

metadata <- data.frame(Time = time, Cells = cells)
...
peaks_dir <- "/mnt/bioinformatics/pipeline/Illumina/220802_VH00126_203_AAAMVKNHV/Project_P452-3Processed_globus_220807/merged_peaks"

peak_files <- list.files(peaks_dir, pattern = "*narrowPeak$", full.names = TRUE)

metadata$Peaks <- peak_files

# metadata df with cols SampleID, Tissue, Factor, Condition, Treatment, Replicate, bamReads (path), ControlID, bamControl, Peaks (bed path), PeakCaller (bed for all?)
# might need few more cols...

metadata <- metadata %>%
  mutate(Replicate = 1) %>%
  dplyr::rename(Condition = Time,
         Tissue = Cells) %>%
  mutate(Factor = NA,
         ControlID = NA,
         bamControl = NA,
...
write.csv(metadata, "../saved_data/merged_samplesheet.csv")
...
pdf("../figures/merged_pca_cells_time.pdf")
print(p)
dev.off()
...
# MA plots are a type of Bland–Altman plot applied to genomic data; MA plots depict global pat- terns of measurements compared between two samples, where each tested genomic feature is quantified by the difference between the two groups as the y-axis (M) and signal intensity as the x-axis
...
saveRDS(model_norm, "../saved_data/merged_normalized_data.rds")
...
#model_norm <- readRDS("../saved_data/merged_normalized_data.rds")
...
pdf("../figures/merged_heatmap_clustering.pdf")
print(p)
dev.off()
...
pdf("../figures/merged_gene_anno_pie.pdf")
print(p)
dev.off()
...
ggsave("../figures/merged_CD57_pos_vs_DN_volcano.svg", p)
...
ggsave("../figures/merged_CD57_neg_vs_DN_volcano.svg", p)
...
ggsave("../figures/merged_CD57_pos_vs_CD57_neg_volcano.svg", p)
