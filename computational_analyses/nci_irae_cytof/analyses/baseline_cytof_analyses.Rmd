...
limma_df <- flow_data %>%
  # subet for baseline cancer data
  filter(cleaned_visit_info == "baseline") %>%
  mutate(freq = logitNorm(freq)) %>%
  select(-c(parent_population, parent_parent_population, irae_ex, ici_drug, data_source)) %>%
  pivot_wider(names_from = sort, values_from = freq) %>%
...
wide_baseline_flow_metadata_confounders_regressed_out <- adjusted_data[, 72:80] %>%
  mutate(patient_id_simple = row_number())

wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 1:71] %>% select(where(~ sum(is.na(.)) <= 1))
...
# Plot PCA results for baseline data with batch regressed out, color by combined irAE yes vs. no, using top features to better separate groups
...
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 1:71] %>% select(where(~ sum(is.na(.)) <= 1)) %>% select(stats_df$sort)
...
ggsave("../figures/baseline_pca_combined_irae_group_batch_regressed_out.png", plot = p, width = 3, height = 3.5)
...
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 1:71] %>% select(where(~ sum(is.na(.)) <= 1)) %>% select(thyroid_stats_df$sort)
...
res <- compareGroups(thyroid_irae ~ PC1 + PC2, data = compVar, p.corrected = TRUE, method = 1)
res
...
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 1:71] %>% select(where(~ sum(is.na(.)) <= 1)) %>% select(thyroid_with_oor_stats_df$sort)
...
p <- ggplot(pcaData, aes(x = thyroid_irae_including_oor, y = PC2, color = thyroid_irae_including_oor)) +
  geom_boxplot() +
...
ggsave("../figures/baseline_pc2_boxplot_thyroid_irae_with_oor_groups_all_features_batch_regressed_out.png", plot = p, width = 5, height = 3.5)

```

# Plot PCA results for baseline data with batch regressed out, skin irAEs specifically, using all features as this contrast actually had results
...
# Plot PCA results for baseline data with batch regressed out, pneumonitis specifically
...
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 1:71] %>% select(where(~ sum(is.na(.)) <= 1)) %>% select(pneumonitis_stats_df$sort)
...
# Plot PCA results for baseline data with batch regressed out, pneumonitis specifically (including effusion)
...
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 1:71] %>% select(where(~ sum(is.na(.)) <= 1)) %>% select(pneumonitis_including_effusion_stats_df$sort)
...
top_contributing_features_PC3_pneumonitis_including_effusion <- pca_contributions %>%
    arrange(desc(Dim.3)) %>%
    select(Dim.3)
...
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 1:71] %>% select(where(~ sum(is.na(.)) <= 1)) %>% select(rheumatoid_irae_stats_df$sort)
...
# Make plot of significant features for specific irAEs
...
plot_data <- adjusted_data %>% select(skin_irae, `CD11cpos of Bcells`, `CM of Tconv Tcells`, `DN of Bcells`, `CM of CD8 Tcells`) %>%
  pivot_longer(cols = c(`CD11cpos of Bcells`, `CM of Tconv Tcells`, `DN of Bcells`, `CM of CD8 Tcells`), names_to = "sort", values_to = "freq")

p <- plot_data %>%
  ggplot(aes(x = skin_irae, y = freq, fill = skin_irae)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(color = "black", size = 1) +
  facet_wrap(~ sort) +
  labs(x = "", y = "Logit-transformed, Z-\nscored % of parent,\nbatch effects removed") +
  scale_fill_manual(values = c("Skin irAE" = "#F8766D", "No skin irAE" = "#00BFC4")) +
  theme(axis.text.x = element_blank(),
        strip.text = element_text(size = 8)) +
  labs(fill = "Skin irAE group")

p

ggsave("../figures/batch_regressed_out_residuals_by_feature_hit_skin_iraes.png", plot = p, width = 6, height = 4)
