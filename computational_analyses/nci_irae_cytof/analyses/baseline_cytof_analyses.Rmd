...
# shuffle points to see if center cluster actually batch-9
set.seed(1)
...
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 17:ncol(adjusted_data)] %>% select(-na_ridden_features) %>% mutate(across(everything(), ~ ifelse(is.na(.), mean(., na.rm = TRUE), .))) %>% select(setdiff(stats_df$sort, na_ridden_features))
...
thyroid_stats_df <- adjusted_data %>%
  ...
  mutate(padj = p.adjust(pval, method = "BH")) %>%
  arrange(pval) %>%
  #head(10) # use for heatmap of combined vs. specific irAE features
  filter(pval < 0.02) # use for PCA, group comparisons
...
# instead of filtering for no NA, remove features in na_ridden_features list and impute rest of NAs
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 17:ncol(adjusted_data)] %>% select(-na_ridden_features) %>% mutate(across(everything(), ~ ifelse(is.na(.), mean(., na.rm = TRUE), .))) %>% select(setdiff(thyroid_stats_df$sort, na_ridden_features))
...
# instead of filtering for no NA, remove features in na_ridden_features list and impute rest of NAs
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 17:ncol(adjusted_data)] %>% select(-na_ridden_features) %>% mutate(across(everything(), ~ ifelse(is.na(.), mean(., na.rm = TRUE), .))) %>% select(setdiff(thyroid_including_oor_stats_df$sort, na_ridden_features))
...
skin_stats_df <- adjusted_data %>%
  ...
  arrange(pval) %>%
  #head(10) # use for heatmap of combined vs. specific irAE features
  filter(pval < 0.1) # use for PCA, group comparisons
...
# instead of filtering for no NA, remove features in na_ridden_features list and impute rest of NAs
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 17:ncol(adjusted_data)] %>% select(-na_ridden_features) %>% mutate(across(everything(), ~ ifelse(is.na(.), mean(., na.rm = TRUE), .))) %>% select(setdiff(skin_stats_df$sort, na_ridden_features))
...
pneumonitis_stats_df <- adjusted_data %>%
  ...
  mutate(padj = p.adjust(pval, method = "BH")) %>%
  arrange(padj) %>%
  head(3) # use for heatmap of combined vs. specific irAE features
...
# instead of filtering for no NA, remove features in na_ridden_features list and impute rest of NAs
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 17:ncol(adjusted_data)] %>% select(-na_ridden_features) %>% mutate(across(everything(), ~ ifelse(is.na(.), mean(., na.rm = TRUE), .))) %>% select(setdiff(pneumonitis_stats_df$sort, na_ridden_features))
...
# instead of filtering for no NA, remove features in na_ridden_features list and impute rest of NAs
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 17:ncol(adjusted_data)] %>% select(-na_ridden_features) %>% mutate(across(everything(), ~ ifelse(is.na(.), mean(., na.rm = TRUE), .))) %>% select(setdiff(rheumatoid_stats_df$sort, na_ridden_features))
...
column_annotations <- data.frame(
  irAE_type = c(rep("combined", 3), rep("skin", 3), rep("thyroid", 3), rep("pneumonitis", 3), rep("rheumatoid", 3)),
