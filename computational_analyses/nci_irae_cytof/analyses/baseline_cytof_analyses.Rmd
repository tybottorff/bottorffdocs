...
feature_of_interest <- "SCM.of.Tconv.Tcells"

adjusted_data %>%
  filter(cleaned_visit_info == "baseline", group %in% c("non-irAE", "irAE/SAE")) %>%
  pivot_longer(cols = -c(patient_id, group, ctcae_grade, Batch, age, thyroid_irae, thyroid_irae_including_oor, skin_irae, pneumonitis_irae, pneumonitis_irae_including_effusion, gastro_irae, rheumatoid_irae, days_to_irae, thyroid_cat, cancer_type, cleaned_visit_info), 
               names_to = "sort", values_to = "freq") %>%
  filter(sort == feature_of_interest) %>% # Select the specific feature
  mutate(ctcae_grade = if_else(is.na(ctcae_grade), 0, ctcae_grade)) %>% # Replace NA with 0 for non-irAE
  ggplot(aes(x = freq, y = ctcae_grade)) +
  geom_point(alpha = 0.7) + # Scatter plot points
  geom_smooth(method = "lm", formula = y ~ x, color = "blue") + # Regression line
  labs(
    x = "SCM of Tconv Tcells (residuals)",
    y = "CTCAE Grade")
