...
# Look at a feature pre/post batch correction
...
plot_data <- flow_data %>%
  ...
  mutate(freq = scale(logitNorm(freq))) %>%
  ungroup() %>%
  filter(sort %in% c("SCM of Tconv Tcells" ,"CD38hiCD127neg of NN CD8 Tcells", "EM of CD8 Tcells", "intermediate of monocytes"),
         cleaned_visit_info == "baseline")
...
p <- plot_data %>%
  ...
  geom_jitter(color = "grey", size = 0.5) +
  labs(x = "", y = "Freq (logit-transformed\n% of parent, Z-scaled)") +
  scale_fill_manual(values = c("Healthy_Control" = "purple", "AID_Patient" = "blue", "non-irAE" = "black", "irAE/SAE" = "red")) +
  theme(axis.text.x = element_blank(),
        strip.text = element_text(size = 8))
...
p <- plot_data %>%
  ...
  facet_wrap(~ sort, ncol = 2) +
  geom_jitter(color = "grey", size = 0.5) +
  labs(x = "", y = "Freq (residuals from LM)") +
  scale_fill_manual(values = c("Healthy_Control" = "purple", "AID_Patient" = "blue", "non-irAE" = "black", "irAE/SAE" = "red")) +
  theme(axis.text.x = element_blank(),
        strip.text = element_text(size = 8))
...
stats_df <- adjusted_data %>%
  ...
  filter(pval < 0.13) # use for PCA, group comparisons
...
thyroid_stats_df <- adjusted_data %>%
  ...
  arrange(pval) %>%
  #head(10) # use for heatmap of combined vs. specific irAE features
  filter(pval < 0.016) # use for PCA, group comparisons
...
skin_stats_df <- adjusted_data %>%
  ...
  arrange(pval) %>%
  #head(10) # use for heatmap of combined vs. specific irAE features
  filter(pval < 0.15) # use for PCA, group comparisons
...
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 17:ncol(adjusted_data)] %>% select(where(~ sum(is.na(.)) <= 1)) %>% select(skin_stats_df$sort)
...
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 17:ncol(adjusted_data)] %>% select(where(~ sum(is.na(.)) <= 1)) %>% select(pneumonitis_stats_df$sort)
...
p <- Heatmap(merged_data, 
        ...
        column_split = ifelse(colnames(merged_data) %in% highlight_columns, "p < 0.1", "p > 0.1"),  # Separate highlighted columns
        show_row_names = TRUE)
