...
flow_data <- read_excel("../raw_data/2024_10_17_flow_data.xlsx") %>%
  ...
  rename(patient_id = `Subject ID`, ctcae_grade = `CTCAE Grade (Highest)`, group = `General irAE Classification`, cancer_type = `Primary Cancer Type`, age = `Age at Draw`, thyroid_cat = `Thyroid Category (JB)`, data_source = `Data Source`, ici_drug = `Immune Checkpoint Inhibitor`, race = Race, sex = `Sex/Gender`) %>%
...
# Plot how data looks for a specific subset pre/post logit

```{r}

specific_subset <- flow_data %>%
  filter(sort %in% c("CD56bright of NKcells", "SCM of Tconv Tcells", "PD1pos of NKcells")) %>%
  mutate(logit_freq = logitNorm(freq)) %>%
  group_by(sort) %>%
  mutate(logit_freq = scale(logit_freq)) %>%
  ungroup()

specific_subset

flow_data_long <- specific_subset %>%
  select(sort, freq, logit_freq) %>%
  pivot_longer(cols = c(freq, logit_freq), names_to = "measure", values_to = "value")

custom_labels <- c(freq = "% of parent", logit_freq = "Transformed %\nof parent")

p <- ggplot(flow_data_long, aes(x = value)) +
  geom_histogram(bins = 30, fill = "skyblue", color = "black") +
  facet_grid(sort ~ measure, scales = "free", labeller = labeller(measure = custom_labels)) +
  labs(x = "Value", y = "Count") +
  theme(strip.text = element_text(size = 8))

p

ggsave("../figures/pre_post_transform_data.png", p, width = 4, height = 5.25)
...
pca_df <- flow_data %>%
  mutate(freq = logitNorm(freq)) %>%
  select(-c(parent_population, parent_parent_population, irae_ex, data_source)) %>%
  pivot_wider(names_from = sort, values_from = freq) %>%
  mutate(Batch = as.factor(Batch),
         patient_id = as.factor(patient_id))

wide_baseline_flow_metadata <- pca_df[, 1:17] %>%
  mutate(patient_id_simple = row_number())

wide_baseline_flow_num_data <- (pca_df)[, 18:ncol(pca_df)] %>%
...
p <- ggplot(data = pcaData) +
  ...
  geom_point(aes(x = PC1, y = PC2, color = Batch), size = 2, alpha = 0.25) + 
...
# Estimate variation contribution from biological & technical sources

```{r}

#wide_baseline_flow_num_data_transposed <- wide_baseline_flow_num_data %>% t()

VarCorr <- lme4::VarCorr

fit <- pvca::PVCA(counts = as.matrix(wide_baseline_flow_num_data_transposed), 
                  meta = wide_baseline_flow_metadata %>% select(patient_id, group, cleaned_visit_info, Batch, age, cancer_type), 
                  inter = FALSE, 
                  threshold = 0.6)
...
# so cancer type and batch are both important covariates to consider in subsequent analyses
# checked race and sex as well, low %s (< 5%) so ok to ignore I think
...
limma_df$group <- factor(limma_df$group)
limma_df$Batch <- factor(limma_df$Batch)

mm_lme <- model.matrix(~ group + Batch + (1|patient_id),
                       data=limma_df$`pDC of nonGrans`)

# Block by donor
consensus.corr <- duplicateCorrelation(
  object = limma_df$freq, 
  design = mm_lme,
  block = limma_df$patient_id)

# View average correlation
consensus.corr$consensus.correlation
## [1] 0.357227
# Fit limma model
fit_lme <- lmFit(object = limma_df$freq, 
                 design = mm_lme,
                 block = limma_df$patient_id,
...
adjusted_data %>%
  pivot_longer(cols = -c(patient_id, group, skin_irae, thyroid_irae, pneumonitis_irae, rheumatoid_irae, ctcae_grade, Batch, cleaned_visit_info), names_to = "sort", values_to = "value") %>%
  group_by(sort) %>%
  summarize(
    Test_Type = if(all(shapiro.test(value[skin_irae == "No skin irAE"])$p.value > 0.05,
                       shapiro.test(value[skin_irae == "Skin irAE"])$p.value > 0.05)) "t-test" else "Wilcoxon",
...
# Try kimma instead as recommended by Naresh
...
kimma_df <- flow_data %>%
  ...
  pivot_wider(names_from = sort, values_from = freq) %>%
  mutate(
    group = factor(group, levels = c("non-irAE", "irAE/SAE", "Healthy_Control", "AID_Patient")),  # Keep all groups as factors
    Batch = as.factor(Batch),
    skin_irae = as.factor(skin_irae),
    pneumonitis_irae = as.factor(pneumonitis_irae),
    rheumatoid_irae = as.factor(rheumatoid_irae),
    thyroid_irae = as.factor(thyroid_irae),
    patient_id = as.factor(patient_id),
    cancer_type = as.factor(cancer_type)) %>%
  mutate(libid = paste(patient_id, cleaned_visit_info, sep = "_")) %>%
  mutate(across(where(is.numeric), ~ scale(.)))

# reorder cols to have all sorts last
kimma_df <- kimma_df[, c(ncol(kimma_df), 1:(ncol(kimma_df)-1))]

model_formula <- "~ group:cleaned_visit_info + Batch + cancer_type + (1|patient_id)"

expr_df <- kimma_df[, 19:ncol(kimma_df)] %>% t()

colnames(expr_df) <- kimma_df$libid

kimma_results <- kmFit(
  counts = expr_df, meta = kimma_df[, 1:18],
  model = model_formula,
  libraryID = "libid",
  run_lme = TRUE,
  run_contrast = TRUE,
  contrast_var = c("group:cleaned_visit_info"),
  patientID = "patient_id"
)

kimma_results$lme.contrast %>%
  # look for just baseline irAE vs. non-irAE differences
  filter(contrast_ref == "(non-irAE baseline)" & contrast_lvl == "(irAE/SAE baseline)" | contrast_lvl == "(non-irAE baseline)" & contrast_ref == "(irAE/SAE baseline)") %>%
  arrange(FDR)
...
lmer_df <- flow_data %>%
  # temporarily filter for baseline data to compare methods of dealing with batch effects
  #dplyr::filter(cleaned_visit_info == "baseline", group %in% c("non-irAE", "irAE/SAE")) %>%
  mutate(freq = logitNorm(freq)) %>%
  select(-c(parent_population, parent_parent_population, irae_ex, ici_drug, data_source)) %>%
  pivot_wider(names_from = sort, values_from = freq) %>%
  mutate(Batch = as.factor(Batch),
         cancer_type = if_else(is.na(cancer_type), "none", cancer_type),
         cancer_type = as.factor(cancer_type)) %>%
  dplyr::filter(!(is.na(patient_id)))

lmer_df

write.csv(lmer_df, "../saved_data/transformed_data.csv")
...
regress_out_confounders <- function(freq, batch, cancer_type, patient_id) {
  # Build the linear model and get residuals
  model <- lmer(freq ~ batch + cancer_type + (1|patient_id))
  residuals(model)
}
...
adjusted_data <- (lapply(feature_cols,
                         ...
                         batch = lmer_df$Batch,
                         cancer_type = lmer_df$cancer_type,
...
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 17:ncol(adjusted_data)] %>% select(-na_ridden_features) %>% mutate(across(everything(), ~ ifelse(is.na(.), mean(., na.rm = TRUE), .))) %>% select(stats_df$sort)
...
p <- ggplot(data = pcaData) +
  geom_hline(yintercept = 0, linetype = 2, color = "grey") +
  geom_vline(xintercept = 0, linetype = 2, color = "grey") +
  geom_point(aes(x = PC1, y = PC3, color = group), size = 2, alpha = 0.8) + 
  stat_ellipse(aes(x = PC1, y = PC3, color = group), size = 1, linetype = 2) +
  labs(color = "") +
  theme_bw() +
  theme(
    legend.position = "bottom", 
...
for (i in 1:num_iterations) {
  ...
  pca_scores$group <- adjusted_data$group
  
  # Test for differences between groups on PCA axes
  pc1_pval <- if (shapiro.test(pca_scores$PC1)$p.value > 0.05) {
    t.test(PC1 ~ group, data = pca_scores)$p.value
  } else {
    wilcox.test(PC1 ~ group, data = pca_scores)$p.value
  }
  
  pc2_pval <- if (shapiro.test(pca_scores$PC2)$p.value > 0.05) {
    t.test(PC2 ~ group, data = pca_scores)$p.value
  } else {
    wilcox.test(PC2 ~ group, data = pca_scores)$p.value
  }
  
  pc3_pval <- if (shapiro.test(pca_scores$PC3)$p.value > 0.05) {
    t.test(PC3 ~ group, data = pca_scores)$p.value
  } else {
    wilcox.test(PC3 ~ group, data = pca_scores)$p.value
  }
...
# so at least 1 < 0.05 and at least 1 < 0.01
# Filter for significant results
significant_results <- results_df %>%
  #filter(PC1_pval < 0.01 | PC2_pval < 0.01 | PC3_pval < 0.01)
  #filter((PC1_pval < 0.05) + (PC2_pval < 0.05) + (PC3_pval < 0.05) >= 2)
  filter(
    (PC1_pval < 0.01 & (PC2_pval < 0.05 | PC3_pval < 0.05)) |
    (PC2_pval < 0.01 & (PC1_pval < 0.05 | PC3_pval < 0.05)) |
    (PC3_pval < 0.01 & (PC1_pval < 0.05 | PC2_pval < 0.05))
  )
...
# Plot distribution of pvals for monte carlo sims

```{r}

monte_carlo_results_df <- pivot_longer(results_df, cols = starts_with("PC"), names_to = "PC", values_to = "pval") %>%
  mutate(PC = sub("_pval", "", PC))

# Plot the histogram faceted by PC
p <- ggplot(monte_carlo_results_df, aes(x = pval)) +
  geom_histogram(bins = 100, fill = "steelblue", color = "black") +
  geom_vline(xintercept = 0.05, color = "red", linetype = "dashed") +
  facet_wrap(~PC, ncol = 1) +
  labs(x = "P value from testing for\ndifference between irAE groups", y = "Random module count")

p

ggsave("../figures/monte_carlo_sims.png", p, width = 5, height = 10)
...
p <- plot_data %>%
  ...
  geom_jitter(color = "black", size = 0.5) +
  facet_wrap(~ sort) +
  labs(x = "", y = "Batch-adjusted,\nnormalized frequency", fill = "Pneumonitis group") +
  theme(axis.text.x = element_blank(),
        strip.text = element_text(size = 12))
...
filtered_data <- merged_data[, colnames(merged_data) %in% highlight_columns, drop = FALSE] %>%
  # get rid of subsets that are always very low weighted in PCs
  filter(if_any(everything(), ~ . > 10))

# Reorder columns: combined, skin, rheumatoid, thyroid, pneumonitis
desired_order <- c(
  "PC1_combined",
  "PC3_combined",
  "PC3_skin",
  "PC1_rheumatoid", "PC3_rheumatoid",
  "PC1_thyroid", "PC2_thyroid",
  "PC1_pneumonitis", "PC2_pneumonitis", "PC3_pneumonitis"
)

filtered_data <- filtered_data[, desired_order, drop = FALSE]
...
# Clean up feature names (replace dots with spaces)
...
column_annotations$irAE_type <- factor(column_annotations$irAE_type, 
                                       levels = c("combined", "skin", "rheumatoid", "thyroid", "pneumonitis"))

# Define colors for column annotations
annotation_colors <- list(
  irAE_type = c("combined" = "black", "skin" = "red", "rheumatoid" = "pink", "thyroid" = "blue", "pneumonitis" = "green"),
...
specific_features <- c("naive of Bcells", "swMem of Bcells", "PD1pos of NKcells", "plasmablasts of Bcells", "CCR4pos of NN Tconv Tcells", "CD4 of Tcells", "gd of Tcells", "Tcells of nonGrans")
...
# split cols as well
#column_split <- factor(c(rep("SCM of Tconv Tcells important", 5), rep("CD56bright of NKcells important", ncol(filtered_data) - 5)),
#                       levels = c("SCM of Tconv Tcells important", "CD56bright of NKcells important"))
...
p <- Heatmap(filtered_data, 
        ...
        heatmap_legend_param = list(title = "PC weights (%)"),
        # split rows based on irAE-type-specific or not
        row_split = row_split,
        #column_split = column_split,
        show_row_names = TRUE,
        column_title = "Subsets in modules for PCs that show irAE group separation")

# Save the heatmap to a PDF file
pdf(file = "../figures/irae_types_contributing_features_highlighted.pdf", width = 10, height = 5)  # Adjust width and height as needed
...
non_feature_cols <- c("patient_id", "group", "specific_group", "irae_ex", "Batch", 
                      "age", "ctcae_grade", "thyroid_irae", "thyroid_irae_including_oor", "skin_irae", "days_to_irae", "thyroid_cat", "cancer_type", "cleaned_visit_info", "pneumonitis_irae", "rheumatoid_irae", "gastro_irae", "pneumonitis_irae_including_effusion")
feature_columns <- setdiff(colnames(df), non_feature_cols)

# Calculate mean residuals for each feature to use for FC
irae_means <- (irae_group %>% select(all_of(feature_columns)) %>% summarise_all(mean, na.rm = TRUE))
no_irae_means <- no_irae_group %>% select(all_of(feature_columns)) %>% summarise_all(mean, na.rm = TRUE)

log2_fc <- log2(as.numeric(irae_means) / as.numeric(no_irae_means))

pvals <- sapply(feature_columns, function(feature) {
  ir_values <- na.omit(irae_group[[feature]])
  no_ir_values <- na.omit(no_irae_group[[feature]])
  
  # Check for normality using Shapiro-Wilk test
  ir_normal <- shapiro.test(ir_values)$p.value > 0.05
  no_ir_normal <- shapiro.test(no_ir_values)$p.value > 0.05

  # Use t.test if both groups are normally distributed, otherwise use wilcox.test
  if (ir_normal && no_ir_normal) {
    t.test(ir_values, no_ir_values)$p.value
  } else {
    wilcox.test(ir_values, no_ir_values)$p.value
  }
})
...
p <- ggplot(volcano_df, aes(x = log2_fc, y = neg_log10_padj)) +
  geom_point(aes(shape = padj < 0.1), color = "black") +
  ...
  geom_text_repel(data = subset(volcano_df, pval < 0.125), 
                  aes(label = feature), size = 3, box.padding = 0.5, force = 3, max.overlaps = 10, nudge_y = 0.05)

p

ggsave("../figures/baseline_combined_irae_vp.png", width = 6, height = 4)
...
# Bar plot of feature importances for combined irAE distinction

```{r}

ggplot(feat_importances$top_features %>% head(10), aes(x = reorder(feature, feature_importance), y = feature_importance)) +
  geom_bar(stat = "identity", fill = "skyblue", color = "black") +
  coord_flip() +
  labs(x = "Subset", y = "Feature Importance") +
  theme(
