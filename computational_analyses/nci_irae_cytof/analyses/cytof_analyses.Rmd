...
# Plot PCA results for baseline data with batch (+/- age) regressed out using top features
...
top_contributing_features_PC3_general <- pca_contributions %>%
    arrange(desc(Dim.3)) %>%
    select(Dim.3)
...
wide_baseline_flow_num_data_confounders_regressed_out <- (adjusted_data)[, 16:ncol(adjusted_data)] %>% select(rheumatoid_irae_stats_df$sort)
...
plot_data <- adjusted_data %>% select(thyroid_irae, CD38hiCD127neg.of.NN.CD8.Tcells) %>%
  pivot_longer(cols = c(CD38hiCD127neg.of.NN.CD8.Tcells), names_to = "sort", values_to = "freq")

p <- plot_data %>%
  ggplot(aes(x = thyroid_irae, y = freq, fill = thyroid_irae)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(color = "grey", size = 0.5) +
  labs(x = "", y = "CD38hi CD127- of NN\nCD8s (residuals)") +
  scale_fill_manual(values = c("No thyroid irAE" = "black", "Thyroid irAE" = "red")) +
  theme(axis.text.x = element_blank(),
        strip.text = element_text(size = 8))

p

ggsave("../figures/batch_regressed_out_residuals_by_feature_hit_thyroid.png", plot = p, width = 7, height = 3.5)

plot_data <- adjusted_data %>% select(skin_irae, CM.of.Tconv.Tcells) %>%
  pivot_longer(cols = c(CM.of.Tconv.Tcells), names_to = "sort", values_to = "freq")

p <- plot_data %>%
  ggplot(aes(x = skin_irae, y = freq, fill = skin_irae)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(color = "grey", size = 0.5) +
  labs(x = "", y = "CM of Tconv\nTcells (residuals)") +
  scale_fill_manual(values = c("No skin irAE" = "black", "Skin irAE" = "red")) +
  theme(axis.text.x = element_blank(),
        strip.text = element_text(size = 8))

p

ggsave("../figures/batch_regressed_out_residuals_by_feature_hit_skin.png", plot = p, width = 7, height = 3.5)

plot_data <- adjusted_data %>% select(pneumonitis_irae, PD1pos.of.NKcells) %>%
  pivot_longer(cols = c(PD1pos.of.NKcells), names_to = "sort", values_to = "freq")
...
p <- plot_data %>%
  ...
  geom_jitter(color = "grey", size = 0.5) +
  labs(x = "", y = "PD1+ of NK\ncells (residuals)") +
  scale_fill_manual(values = c("No pneumonitis" = "black", "Pneumonitis" = "red")) +
  theme(axis.text.x = element_blank(),
        strip.text = element_text(size = 8))

p

ggsave("../figures/batch_regressed_out_residuals_by_feature_hit_pneumonitis.png", plot = p, width = 7, height = 3.5)

plot_data <- adjusted_data %>% select(rheumatoid_irae, naive.of.Bcells) %>%
  pivot_longer(cols = c(naive.of.Bcells), names_to = "sort", values_to = "freq")

p <- plot_data %>%
  ggplot(aes(x = rheumatoid_irae, y = freq, fill = rheumatoid_irae)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(color = "grey", size = 0.5) +
  labs(x = "", y = "Naive of B\ncells (residuals)") +
  scale_fill_manual(values = c("No rheumatoid irAE" = "black", "Rheumatoid irAE" = "red")) +
  theme(axis.text.x = element_blank(),
        strip.text = element_text(size = 8))

p

ggsave("../figures/batch_regressed_out_residuals_by_feature_hit_rheumatoid.png", plot = p, width = 7, height = 3.5)
