...
# Function to regress out confounders (donorId, age, batch) for each feature
regress_out_confounders <- function(feature, donorId, age, batch) {
  # Build the linear model and get residuals
  model <- lm(feature ~ age + batch)
  residuals(model)
}

adjusted_features <- as.data.frame(lapply(feature_cols, 
                                          regress_out_confounders, 
                                          age = regressing_out_confounders_df$age, 
...
p <- ggplot(data = pcaData) +
  geom_hline(yintercept = 1, linetype = 2, color = "grey") +
  geom_vline(xintercept = 0, linetype = 2, color = "grey") +
  geom_point(aes(x = PC1, y = PC2, color = age), size = 2, alpha = 0.8) + 
  #scale_color_manual(values = c("Healthy_Control" = "purple", "AID_Patient" = "blue", "non-irAE" = "black", "irAE/SAE" = "red")) +
  scale_color_gradient2(low = "blue", mid = "grey", high = "red", midpoint = median(pcaData$age, na.rm = TRUE), limits = c(min(pcaData$age), max(pcaData$age))) +  # Blue-White-Red gradient with median age as white
  xlim(-limits[1], limits[1]) +
  ylim(-limits[2], limits[2]) +
...
#ggsave("../figures/baseline_pca_general_group_age_batch_regressed_out.png", plot = p, width = 3, height = 3.5)
ggsave("../figures/baseline_pca_age_with_age_batch_regressed_out.png", plot = p, width = 3, height = 3.5)
