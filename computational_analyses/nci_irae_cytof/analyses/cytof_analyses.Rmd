...
baseline_flow_data <- read_excel("../raw_data/2024_10_17_flow_data.xlsx") %>%
  ...
  rename(patient_id = `Subject ID`, ctcae_grade = `CTCAE Grade (Highest)`, group = `General irAE Classification`, specific_group = `Patient Type`, age = `Age at Draw`) %>%
  ...
  select(patient_id, group, ctcae_grade, specific_group, cleaned_visit_num, Batch, age, starts_with("pct_")) %>%
  pivot_longer(
...
stats_test <- baseline_flow_data %>%
  ...
  filter(!(is.na(irae_severity_based_on_ctcae))) %>%
  group_by(sort) %>%
  summarize(pval = wilcox.test(freq[irae_severity_based_on_ctcae == "Non-irAE"], freq[irae_severity_based_on_ctcae == "Non-serious irAE/SAE"], paired = FALSE, exact = FALSE)$p.value) %>%
...
wide_baseline_flow_num_data <- baseline_flow_data %>%
  ...
  select(-c(patient_id, group, specific_group, Batch, age, irae_severity_based_on_ctcae, ctcae_grade)) %>%
...
wide_baseline_flow_metadata <- baseline_flow_data %>%
  ...
  select(-c(parent_population, parent_parent_population, cleaned_visit_num)) %>%
  pivot_wider(names_from = sort, values_from = freq) %>%
  drop_na(-c(group, patient_id, Batch, age, irae_severity_based_on_ctcae, ctcae_grade)) %>%
  select(c(patient_id, group, specific_group, Batch, age, irae_severity_based_on_ctcae, ctcae_grade)) %>%
...
regressing_out_confounders_df <- baseline_flow_data %>%
  select(-c(parent_population, parent_parent_population, cleaned_visit_num, age, irae_severity_based_on_ctcae)) %>%
  pivot_wider(names_from = sort, values_from = freq) %>%
  drop_na(-c(group, specific_group, ctcae_grade)) %>%
...
wide_baseline_flow_metadata_confounders_regressed_out <- adjusted_data[, 1:5] %>%
  mutate(patient_id_simple = row_number())

wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 6:ncol(adjusted_data)]
