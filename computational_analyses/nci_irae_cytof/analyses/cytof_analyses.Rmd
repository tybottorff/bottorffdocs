...
p <- ggplot(data = pcaData) +
  ...
  scale_color_manual(values = c("Cancer group - no irAE" = "black", "Not severe" = "green", "Severe" = "red")) +  # Set custom colors
...
# Try mixed effects modeling like in https://www.biorxiv.org/content/10.1101/2022.06.05.494592v1.full.pdf

```{r}

mixed_effects_model_df <- cancer_flow_data %>%
  group_by(sort) %>%
  filter(all(freq > 0)) %>%
  ungroup() %>%
  # getting rid of any sorts with any NA freqs gets rid of 27/80 sorts
  mutate(visit = case_when(
           cleaned_visit_num == 1 ~ "Pre-ICI",
           cleaned_visit_num %in% c(2, 3) ~ "Post-ICI"),
         patient_id_group_time = paste(patient_id, group, visit, sep = "_"),
         group.time = paste(group, visit, sep = "."))

mixed_effects_model_wide_df <- mixed_effects_model_df %>%
  select(patient_id_group_time, freq, sort) %>%
  pivot_wider(names_from = patient_id_group_time, 
              values_from = freq)

mixed_effects_model_matrix <- as.matrix(mixed_effects_model_wide_df %>% select(-sort))

rownames(mixed_effects_model_matrix) <- mixed_effects_model_wide_df$sort

library(edgeR)

d0 <- DGEList(mixed_effects_model_matrix)

d0 <- calcNormFactors(d0)

d0$samples$patient_id_group_time <- as.factor(rownames(d0$samples))

d0$samples <- d0$samples %>%
  mutate(patient_id = as.factor(str_extract(patient_id_group_time, "^[^_]+")),
         visit = as.factor(str_extract(patient_id_group_time, "(?<=_)Pre-ICI|(?<=_)Post-ICI")),
         irae_group = as.factor(str_extract(patient_id_group_time, "(?<=_)[^_]+(?=_)")),
         group.time = as.factor(paste(irae_group, visit, sep = ".")))

d0$samples

# fixed effect model (no patient ID)
mm <- model.matrix(~0 + group.time, data = d0$samples)

y <- voom(d0, mm, plot = T)

# mixed effects model, allow for variation between subjects at baseline, 1|patient_id doesn't account for variation b/w subjects in response but group.time does (variation between irAE groups in response to ICI over time)
library(lme4)
fit <- lm(freq ~ 0 + group.time, data = as.data.frame(mixed_effects_model_df))

# use a constrast matrix L
L <- matrix(c(
  -0.5, 0.5, -0.5, 0.5,  # ICI treatment effects
  -1, 1, 1, -1,          # ICI-associated irAE effects
  1, 0, -1, 0            # Baseline effects
), nrow = 3, byrow = TRUE)
# 4 levels are
# 1 = irAE baseline, 2 = irAE post-avelumab, 3 = no irAE baseline, 4 = no irAE post-avelumab (columns in matrix)
