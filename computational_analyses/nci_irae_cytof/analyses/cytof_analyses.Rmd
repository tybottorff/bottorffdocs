...
baseline_flow_data <- read_excel("../raw_data/holly_flow_data_all_visits.xlsx") %>%
  ...
  rename(patient_id = `Subject ID`, group = `General irAE Classification 9-8-24`, specific_group = `Patient Type`, irae_ex = `Documented irAE's (9/6/2024)`, age = `Age at Draw`, radiation = `Cancer Treated with Radiation?`, chemoradiation = `Cancer Treated with Chemoradiation?`, intravesicular_bcg = `Cancer Treated with Intravesicular BCG?`, intravesicular_chemo = `Cancer Treated with Intravesicular Chemotherapy?`) %>%
  dplyr::filter(cleaned_visit_num == 1 | is.na(cleaned_visit_num) & group %in% c("AID_Patient", "Healthy_Control")) %>%
  select(patient_id, group, specific_group, cleaned_visit_num, irae_ex, Batch, age, radiation, chemoradiation, intravesicular_bcg, intravesicular_chemo, starts_with("pct_")) %>%
  pivot_longer(
...
stats_test <- baseline_flow_data %>%
  ...
  group_by(sort) %>%
  summarize(pval = wilcox.test(freq[group == "irAE/SAE"], freq[group == "non-irAE"], paired = FALSE, exact = FALSE)$p.value) %>%
...
# 71 features, most sig (but padj > 0.15) are PD1pos of CM Tconv Tcells, Tph of NN Tconv Tcells...
# 43 patients...
...
wide_baseline_flow_metadata <- baseline_flow_data %>%
  ...
  #       specific_group == "Type 1 Diabetes" ~ "T1D",
  #       specific_group == "RA" ~ "RA",
  #       TRUE ~ "Gut")) %>%
  #mutate(specific_group = case_when(
  ...
  select(-c(parent_population, parent_parent_population, cleaned_visit_num)) %>%
  pivot_wider(names_from = sort, values_from = freq) %>%
  drop_na(-c(group, irae_ex, irae_severity_category, pneumonitis, thyroid_irae, skin_irae, Batch, age, radiation, chemoradiation, intravesicular_bcg, intravesicular_chemo)) %>%
  select(c(patient_id, group, specific_group, irae_ex, irae_severity_category, Batch, age, radiation, chemoradiation, intravesicular_bcg, intravesicular_chemo)) %>%
...
mixed_effects_model_df <- read_excel("../raw_data/holly_flow_data_all_visits.xlsx") %>%
  rename(patient_id = `Subject ID`, group = `General irAE Classification 9-8-24`, specific_group = `Patient Type`, irae_ex = `Documented irAE's (9/6/2024)`, visit_info = `Visit Descriptor`) %>%
  select(patient_id, group, specific_group, irae_ex, visit_info, Batch, starts_with("pct_")) %>%
  pivot_longer(
    cols = starts_with("pct_"), 
    names_to = "sort",
    values_to = "freq") %>%
...
for (s in sorts) {
  ...
  unique_combinations <- subset_data %>%
    select(patient_id, visit_info, group)
  
  # Fit the mixed-effects model
  fit <- lmer(freq ~ group * visit_info + Batch + (1 | patient_id), data = subset_data)
...
# Look for baseline effects subtracting group/time effects, observe none

```{r}

combined_residuals_df %>%
  filter(group %in% c("non-irAE", "irAE/SAE") & visit_info == 1) %>%
  group_by(sort) %>%
  summarize(pval = wilcox.test(residuals[group == "irAE/SAE"], residuals[group == "non-irAE"], paired = FALSE, exact = FALSE)$p.value) %>%
  mutate(adj_pval = p.adjust(pval, method = "BH")) %>%
  arrange(adj_pval) %>%
  print()
...
"severe vs. nonsevere/no irAE over time
CD4 of Tcells
EMRA of CD8 Tcells
EM of CD8 Tcells
CD8 of Tcells"
...
p <- ggplot(mixed_effects_model_df %>% filter(sort %in% c("CD4 of Tcells", "CD8 of Tcells", "EM of CD8 Tcells", "EMRA of CD8 Tcells")), aes(x = interaction(visit_info, irae_severity_category), y = (freq), color = irae_severity_category)) +
...
# 12 cancer patients lost because of > 1 NA in > 1 feature

regressing_out_confounders_df <- baseline_flow_data %>%
  select(-c(parent_population, parent_parent_population, cleaned_visit_num, irae_ex, age, radiation, chemoradiation, intravesicular_bcg, intravesicular_chemo, irae_severity_category, pneumonitis, thyroid_irae, skin_irae)) %>%
  pivot_wider(names_from = sort, values_from = freq) %>%
  drop_na(-c(group, specific_group)) %>%
  mutate(Batch = as.factor(Batch))

regressing_out_confounders_df

metadata_cols <- regressing_out_confounders_df[, 1:14]
feature_cols <- regressing_out_confounders_df[, 15:ncol(regressing_out_confounders_df)]
...
adjusted_features <- as.data.frame(lapply(feature_cols, 
...
# Plot PCA results for baseline data with batch regressed out

```{r}

wide_baseline_flow_metadata_confounders_regressed_out <- adjusted_data[, 1:14] %>%
  mutate(patient_id_simple = row_number())

wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 15:ncol(adjusted_data)]
...
#ggsave("../figures/baseline_pca_batch_with_batch_regressed_out.png", plot = p, width = 3, height = 3.5)
...
irae_group <- df %>% filter(group == "irAE/SAE")
no_irae_group <- df %>% filter(group == "non-irAE")

non_feature_cols <- c("patient_id", "group", "specific_group", "irae_ex", "Batch", 
                      "age", "radiation", "chemoradiation", "intravesicular_bcg", 
                      "intravesicular_chemo", "irae_severity_category", "pneumonitis", 
                      "thyroid_irae", "skin_irae")
...
fc <- (as.numeric(irae_means) / as.numeric(no_irae_means))
log2_fc <- sign(fc) * log2(abs(fc))

pvals <- sapply(feature_columns, function(feature) {
  t.test(irae_group[[feature]], no_irae_group[[feature]])$p.value
})
...
ggplot(volcano_df, aes(x = log2_fc, y = neg_log10_padj)) +
  geom_point(aes(color = padj < 0.05)) +
  theme_minimal() +
  labs(x = "irAE/SAE vs. no irAE FC (log2)", y = "-log10(padj)") +
  geom_vline(xintercept = 0, linetype = "dashed") + # Vertical line at 0 fold change +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed") + # horizontal line at sig threshold
  geom_text_repel(data = subset(volcano_df, neg_log10_padj > 0.5 & abs(log2_fc) > 2), 
...
adjusted_data %>% select(group, KLRG1posTIGITpos.of.NN.CD8.Tcells) %>%
  pivot_longer(cols = KLRG1posTIGITpos.of.NN.CD8.Tcells, names_to = "sort", values_to = "freq") %>%
  ggplot(aes(x = group, y = freq, fill = group)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(color = "grey") +
  labs(x = "", y = "TIGIT+KLRG1+ of NN\nCD8 Tcells residuals\n(batch regressed out)") +
...
pvals <- sapply(feature_columns, function(feature) {
  wilcox.test(irae_group[[feature]], non_irae_group[[feature]], paired = FALSE, exact = FALSE)$p.value
})
