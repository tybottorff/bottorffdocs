...
stats_test <- baseline_flow_data %>%
  ...
  group_by(sort) %>%
  # dropping the 12 patients also results in significance for PD1+ of CM Tconv Tcells here...
  #filter(patient_id %in% adjusted_data$patient_id) %>%
  summarize(
...
regressing_out_confounders_df <- baseline_flow_data %>%
  ...
  pivot_wider(names_from = sort, values_from = freq) %>%
  # don't drop NA rows if instead removing features (cols)
  drop_na(-c(group, specific_group, ctcae_grade)) %>%
  mutate(Batch = as.factor(Batch))

regressing_out_confounders_df

metadata_cols <- regressing_out_confounders_df[, 1:6]
feature_cols <- regressing_out_confounders_df[, 7:ncol(regressing_out_confounders_df)]

# could also try removing 14/71 features with NAs instead of removing the patients
...
wide_baseline_flow_metadata_confounders_regressed_out <- adjusted_data[, 1:6] %>%
  mutate(patient_id_simple = row_number()) %>%
  mutate(group = case_when(
    group == "Healthy_Control" ~ "HC",
    group == "AID_Patient" ~ "AID",
  ...
  subgroup = case_when(
    ...
    group == "AID" & str_detect(specific_group, "Ulcerative Colitis") ~ "Gut",
...
wide_baseline_flow_num_data_confounders_regressed_out <- adjusted_data[, 7:ncol(adjusted_data)]
...
#ggsave("../figures/baseline_pca_aid_subgroup_batch_regressed_out.png", plot = p, width = 3, height = 3.5)
...
ggplot(volcano_df, aes(x = abs_diff, y = neg_log10_padj)) +
  ...
  geom_text_repel(data = subset(volcano_df, padj < 0.1), 
...
p <- adjusted_data %>% select(group, PD1posTIGITpos.of.EM.Tconv.Tcells, PD1pos.of.CM.Tconv.Tcells, KLRG1posTIGITpos.of.NN.CD8.Tcells) %>%
  pivot_longer(cols = c(PD1posTIGITpos.of.EM.Tconv.Tcells, PD1pos.of.CM.Tconv.Tcells, KLRG1posTIGITpos.of.NN.CD8.Tcells), names_to = "sort", values_to = "freq") %>%
  ggplot(aes(x = group, y = freq, fill = group)) +
  geom_boxplot(outlier.shape = NA) +
  facet_wrap(~ sort, ncol = 3) +
  geom_jitter(color = "grey", size = 0.5) +
  labs(x = "", y = "Residuals (batch regressed out)") +
...
ggsave("../figures/batch_regressed_out_residuals_by_feature_hits.png", plot = p, width = 10, height = 5)
