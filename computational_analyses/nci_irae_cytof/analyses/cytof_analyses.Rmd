...
time_to_irae_df <- read_excel("../raw_data/2024_10_17_flow_data.xlsx") %>%
  filter(`General irAE Classification` == "irAE/SE",
         `Visit Descriptor` %in% c("Visit 1", "visit 1")) %>%
  ...
  select(`Subject ID`, days_to_irae) %>%
  filter(!(is.na(days_to_irae))) %>%
  rename(patient_id = `Subject ID`) %>%
  left_join(baseline_flow_data, by = "patient_id") %>%
  group_by(sort) %>%
