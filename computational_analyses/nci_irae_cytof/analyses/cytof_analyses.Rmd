...
regressing_out_confounders_df <- baseline_flow_data %>%
  select(-c(parent_population, parent_parent_population, cleaned_visit_num)) %>%
  pivot_wider(names_from = sort, values_from = freq) %>%
  drop_na(-c(group, specific_group, irae_ex, Batch, age, radiation, chemoradiation, intravesicular_bcg, intravesicular_chemo, irae_severity_category, pneumonitis, thyroid_irae, skin_irae)) %>%
...
ggplot(volcano_df, aes(x = log2_fc, y = neg_log10_padj)) +
  ...
  geom_text_repel(data = subset(volcano_df, neg_log10_padj > 0.5 & abs(log2fc) > 2), 
                  aes(label = feature), size = 3, box.padding = 0.5, max.overlaps = 10)

```

# Make a boxplot for 1 ~significant (after FDR) feature

```{r}

adjusted_data %>% select(non_feature_cols, PD1pos.of.CM.Tconv.Tcells) %>%
  pivot_longer(cols = PD1pos.of.CM.Tconv.Tcells, names_to = "sort", values_to = "freq") %>%
  ggplot(aes(x = group, y = freq, fill = group)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(color = "grey") +
  labs(x = "", y = "PD1+ of CM Tconv Tcells\nresiduals (batch regressed out)") +
