...
cohort_metadata_long <- baseline_flow_data %>%
  ...
  slice(1) %>%
  ungroup() %>%
  dplyr::rename(cancer_type = specific_group) %>%
  dplyr::filter(group %in% c("irAE/SAE", "non-irAE")) %>%
  # fix days to irae (also was days to thyroid test)
  mutate(days_to_irae = if_else(group == "non-irAE", NA, days_to_irae),
         irae_ex = if_else(group == "non-irAE", "None", irae_ex),
         irae_ex = if_else(group == "irAE/SAE" & irae_ex == "None", "Unspecified", irae_ex)) %>%
  # Split cancer_type and irae_ex into multiple rows for plotting separate points
  separate_rows(cancer_type, sep = ",\\s*") %>%
  separate_rows(irae_ex, sep = ";\\s*") %>%
  # Standardize values for cancer types and toxicities
  mutate(
    cancer_type = case_when(
      grepl("Head|Neck", cancer_type) ~ "Head/neck",
      grepl("Lung", cancer_type) ~ "Lung",
      grepl("Urinary|Urethral|Bladder|Renal", cancer_type) ~ "Urethral/bladder/renal",
      grepl("Gastric", cancer_type) ~ "Gastric",
      grepl("Esophageal", cancer_type) ~ "Esophageal",
      TRUE ~ cancer_type
    ),
    irae_ex = case_when(
      grepl("Rash", irae_ex) ~ "Rash",
      grepl("Thyroid|Hypothyroidism|Hyperthyroidism", irae_ex) ~ "Thyroid",
      grepl("Arthralgia|arthritis", irae_ex) ~ "Arthritis",
      grepl("Encephalitis", irae_ex) ~ "Encephalitis",
      grepl("Pruritus", irae_ex) ~ "Pruritus",
      grepl("Pericardial/Pleural effusion", irae_ex) ~ "Pericardial/pleural effusion",
      grepl("edema", irae_ex) ~ "Limb edema",
      grepl("Effusion", irae_ex) ~ "Effusion",
      grepl("arthalgia", irae_ex) ~ "Arthralgia",
      TRUE ~ irae_ex
    ),
  ...
  pivot_longer(cols = c(cancer_type, irae_ex, ctcae_grade, age, days_to_irae),
               names_to = "variable", values_to = "value") %>%
  mutate(
    variable = factor(variable, levels = c("cancer_type", "irae_ex", "ctcae_grade", "age", "days_to_irae")),
  ) %>%
...
# Create ggplot with multiple 'y' variables
p <- ggplot(cohort_metadata_long %>% dplyr::filter(variable %in% c("Cancer\ntype", "irAE\ntype", "CTCAE\ngrade")), aes(x = patient_id, y = value)) +
  geom_point(aes(color = group), size = 1.5, alpha = 0.75) +
