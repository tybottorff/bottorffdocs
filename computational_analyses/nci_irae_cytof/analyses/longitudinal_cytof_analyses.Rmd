...
# Look at how immmunotypes shift over time in cancer irAE/no irAE groups vs. AID
 - using PCA first, this doesn't feel as right but is useful for presentation figs
...
# just show 1 longitudinal point and centroids for projection analysis (non-irAE group for this patient)
no_irae_centroid_df <- data.frame(PC1 = no_irae_centroid[1], PC2 = no_irae_centroid[2], label = "non-irAE centroid (baseline)")
...
p <- ggplot(data = pcaData %>% filter(patient_id == "LinsleyLab163195" & days_from_baseline != 0)) +
  ...
  geom_vline(xintercept = 0, linetype = 2, color = "grey") +
  geom_point(aes(x = PC1, y = PC2, color = group), size = 2, alpha = 0.8) + 
  geom_text(aes(x = PC1, y = PC2, color = group, label = "Longitudinal point"), size = 3, vjust = -1, hjust = -0.15) +
  stat_ellipse(aes(x = PC1, y = PC2, color = group), size = 1, linetype = 2) +
...
ggsave("../figures/aid_cancer_baseline_pca_example_point.svg", p, width = 3, height = 4)
