  ...
  tryCatch(
    {
      ...
      model_interaction <- gam(freq ~ pneumonitis_irae + 
                                 s(days_from_baseline, k = 5, bs = "cr") + 
                                 s(days_from_baseline, k = 5, bs = "cr", by = pneumonitis_irae) + 
                                 s(patient_id, bs = "re"),
                               data = data, method = 'REML')
      
      # Model without interaction between irAE group and time from baseline (common smooth for both groups)
      model_no_interaction <- gam(freq ~ pneumonitis_irae + 
                                    s(days_from_baseline, k = 5, bs = "cr") + 
                                    s(patient_id, bs = "re"),
                                  data = data, method = 'REML')
...
# just focusing on CD56bright of NK after fixing typo in GAM (Naresh pointed out at WIP practice talk)

# combined has other results besides CD56bright of NK
# skin only has CD56bright of NK
# rheumatoid: NONE
# thyroid: stuff but no CD56bright of NK
# pneumonitis: 2 results but no CD56bright of NK
