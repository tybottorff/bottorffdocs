...
saveRDS(just_T_cells, "../saved_data/final_seurat_T_cells.rds")
#just_T_cells <- readRDS("../saved_data/final_seurat_T_cells.rds")
...
VlnPlot(non_naive_cd8s, features = c("TIGIT", "KLRG1", "EOMES"), group.by = "irAE", pt.size = 0)

```

# Look for TIGIT+KLRG1+ frequency differences by irAE group

```{r}

group_data <- non_naive_cd8s@meta.data %>% dplyr::select(irAE, orig.ident)
...
tigit_klrg1_counts_and_group_data <- group_data %>%
  ...
  group_by(irAE, orig.ident) %>%
  summarize(total_count = n(),
            double_pos_count = sum(TIGIT_status == "pos" & KLRG1_status == "pos")) %>%
  mutate(freq = double_pos_count / total_count) %>%
  dplyr::select(irAE, orig.ident, freq)

tigit_klrg1_counts_and_group_data

wilcox.test(tigit_klrg1_counts_and_group_data$freq ~ tigit_klrg1_counts_and_group_data$irAE)

ggplot(tigit_klrg1_counts_and_group_data, aes(x = irAE, y = freq, color = irAE)) +
  geom_boxplot() + 
  labs(x = "irAE status", y = "TIGIT+ KLRG1+\nnon-naive CD8 frequency", color = "irAE status")

```

# Stats analysis for CD8 non-naive T cells between irAE groups, pseudobulk, specifically looking at TIGIT, KLRG1, EOMES

```{r}

non_naive_cd8s_pseudobulk <- AggregateExpression(non_naive_cd8s, assays = "integrated", return.seurat = T, group.by = c("orig.ident", "irAE"))

non_naive_cd8s_pseudobulk$irAE <- factor(non_naive_cd8s_pseudobulk$irAE, levels = c("Yes", "No"))

Idents(non_naive_cd8s_pseudobulk) <- "irAE"

Idents(non_naive_cd8s_pseudobulk)
