...
tigit_klrg1_counts_and_group_data <- group_data %>%
  ...
  group_by(irAE, orig.ident, TIGIT_status, KLRG1_status) %>%
  summarize(count = n()) %>%
  filter(TIGIT_status == "pos",
         KLRG1_status == "pos")

tigit_klrg1_counts_and_group_data

wilcox.test(tigit_klrg1_counts_and_group_data$count ~ tigit_klrg1_counts_and_group_data$irAE)

ggplot(tigit_klrg1_counts_and_group_data, aes(x = irAE, y = count, color = irAE)) +
  geom_boxplot() + 
