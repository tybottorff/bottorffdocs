...
neoantigen_tcrs <- read.csv("../../Phenotypic_signatures_of_circulating_neoantigen_reactive_CD8_T_cells_in_patients_with_metastatic_cancers/saved_data/joined_tcrs.csv") %>%
  select(-X) %>%
  filter(TRA.cdr3 %in% c("CAVRDNYGQNFVF", "CAVVDNYGQNFVF"))

neoantigen_tcrs

fresh_tcr_seq <- read.csv("../saved_data/tcr_seq_features.csv") %>%
  select(-X) %>%
  filter(Chain == "TRA" & cdr3 %in% neoantigen_tcrs$TRA.cdr3 & v_gene == "TRAV21")
