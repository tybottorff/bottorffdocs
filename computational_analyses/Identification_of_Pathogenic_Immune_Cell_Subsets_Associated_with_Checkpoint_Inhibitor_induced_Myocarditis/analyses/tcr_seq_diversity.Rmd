...
cdr3_amino_acid_diversity_df <- dehashed_combined_tcr_seq_doubled %>%
  ...
  group_by(cdr3, chain, Patient_ID, irAE, predicted.celltype.l2) %>%
  ...
  slice(1) %>%
  ungroup() %>%
  select(chain, irAE, Patient_ID, shannon, inv_shannon, predicted.celltype.l2)
  ...
  #        NA
  #      } else {
  #        wilcox.test(shannon ~ irAE, exact = FALSE)$p.value
  #      }
  #    }) %>%
  #ungroup() %>%
...
ggplot(cdr3_amino_acid_diversity_df %>% filter(predicted.celltype.l2 %in% c("CD4 Proliferating", "CD8 Naive", "CD8 TEM", "Treg")), aes(x = chain, y = shannon, fill = irAE)) +
  geom_boxplot() +
