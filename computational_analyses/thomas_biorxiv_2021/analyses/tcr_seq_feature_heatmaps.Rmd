...
expansion_df <- combined_data %>%
  pivot_wider(id_cols = c(barcode, patient_id, group, predicted.celltype.l1, predicted.celltype.l2), names_from = "Chain", values_from = c("v_gene", "j_gene", "c_gene", "cdr3", "pgen", "cdr3_length", "cdr3_hydrophobicity")) %>%
...
prepped_data <- expansion_df %>%
  ...
  left_join(seq_depth, by = c("patient_id", "group")) %>%
