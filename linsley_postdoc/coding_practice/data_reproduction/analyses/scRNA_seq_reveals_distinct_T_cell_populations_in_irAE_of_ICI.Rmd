...
merged_seurat <- subset(merged_seurat, subset = nFeature_RNA > 500 & percent.mt < 20)
...
#merged_seurat <- ScaleData(merged_seurat, vars.to.regress = c("percent.mt"))
...
merged_seurat <- SCTransform(merged_seurat, variable.features.n = 3000)
...
# can also see which genes are contributing most to each PC, seeing some MT genes despite regressing out percent.mt...
