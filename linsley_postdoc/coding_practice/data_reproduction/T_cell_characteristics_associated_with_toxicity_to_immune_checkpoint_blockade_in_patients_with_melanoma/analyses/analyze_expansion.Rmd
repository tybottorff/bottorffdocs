...
clonality_subcluster_df <- clonality_subcluster_df %>%
  ...
  select(subcluster, CDR3.AA.Sequence, CDR3.NT.Sequence, CR.type) %>%
  filter(grepl("CD4", subcluster),
         !(is.na(CDR3.NT.Sequence)),
         CR.type == "TCR") %>%
  group_by(subcluster, CDR3.NT.Sequence) %>%
  mutate(cdr3_nt_count = n()) %>%
  ungroup() %>%
  group_by(subcluster, CDR3.AA.Sequence) %>%
  ...
  slice(1) %>%
  ungroup() %>%
  select(subcluster, shannon_nt, shannon_aa, complement_score_nt, complement_score_aa) %>%
  pivot_longer(
...
# Plot clonality scores across CD4 T cell subclusters

```{r}

clonality_subcluster_df %>%
  filter(origin == "nt") %>%
  ggplot(aes(x = complement_score, y = shannon_score, color = subcluster)) +
  geom_point() +
