...
ggplot(new_convergence_df, aes(x = nts_per_aa, fill = group)) +
  geom_histogram(position = "dodge", binwidth = 1) +
  facet_wrap(~predicted.celltype.l1) +
  labs(x = "CDR3 nt seqs / CDR3 AA seq", y = "count (log10)") +
  scale_y_continuous(trans = "log10", expand = expansion(add = c(0, 0))) +
  scale_x_continuous(breaks = pretty_breaks(n = 3))

wilcox.test(nts_per_aa ~ group, data = new_convergence_df %>% filter(predicted.celltype.l1 == "CD4 T"))
# 

wilcox.test(nts_per_aa ~ group, data = new_convergence_df %>% filter(predicted.celltype.l1 == "CD8 T"))
# 

wilcox.test(nts_per_aa ~ group, data = new_convergence_df %>% filter(predicted.celltype.l1 == "other T"))
# 

#p.adjust(c("0.1349", "0.02999", "0.1628"), method = "BH")
