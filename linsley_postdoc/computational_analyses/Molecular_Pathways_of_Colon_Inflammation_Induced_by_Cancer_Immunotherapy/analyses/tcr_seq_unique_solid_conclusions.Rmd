...
irae_convergence_df <- convergence_across_patients %>%
  select(barcode, Chain, cdr3, predicted.celltype.l1, predicted.celltype.l2, group, cdr3_nt, distinct_cdr3_nt_count, clonotype) %>%
  group_by(clonotype) %>%
  slice(1)

irae_convergence_df

p <- ggplot(irae_convergence_df, aes(x = predicted.celltype.l1, y = log10(distinct_cdr3_nt_count), fill = group)) +
  geom_boxplot() +
...
ggplot(donor_df %>% filter(group != "no ICI"), aes(x = donors_per_junction_aa, y = donors_per_junction_nt)) +
  geom_point(alpha = 0.25) +
  geom_abline(intercept = 0, slope = 1, color = "red", linetype = "dashed") +
  facet_wrap(~tag + group, ncol = 4) +
