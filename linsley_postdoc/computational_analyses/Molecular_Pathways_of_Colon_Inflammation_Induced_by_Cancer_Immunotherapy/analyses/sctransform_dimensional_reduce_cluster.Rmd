...
starting_letters <- substr(names(colon_t_cells_filtered.list), 1, 1)

# Create a data frame with Seurat object names and their starting letters
seurat_info <- data.frame(name = names(colon_t_cells_filtered.list), starting_letter = starting_letters)
...
immune.anchors <- FindIntegrationAnchors(object.list = colon_t_cells_filtered.list, reference = which(names(colon_t_cells_filtered.list) %in% randomly_selected_seurat$name), normalization.method = "SCT", anchor.features = features, reduction = "rpca", k.anchor = 10, dims = 1:30)
