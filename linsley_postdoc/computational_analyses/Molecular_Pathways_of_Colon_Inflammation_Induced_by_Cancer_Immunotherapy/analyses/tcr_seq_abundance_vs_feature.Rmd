...
cell_abundance_df <- feature_df %>%
  ...
  filter(predicted.celltype.l2 %in% c("CD4 Proliferating", "CD8 Naive", "CD4 TEM", "CD8 TCM", "MAIT"),
         group != "no ICI")
...
cell_abundance_df_downsampled <- downsampled_df %>%
  ...
  ungroup() %>%
  # filter for just subtypes of interest
  filter(predicted.celltype.l2 %in% c("CD4 Proliferating", "CD4 TEM", "CD8 TCM", "MAIT", "CD8 Naive"),
         group != "no ICI")
...
p_values <- c(0.0632, 0.004662, 0.4908, 0.000666, 0.007992, 0.04949, 0.004662, 0.651, 0.219, 0.000666, 0.5728)
...
# Re-do cell abundance comparisons with new Trm clusters

 - change clusters 25 and 27 (CD4 TCM) to CD4 Trm and clusters 3, 7, 9, 15-16 (CD8 TEM) to CD8 Trm
...
cell_abundance_df <- trm_df %>%
  group_by(patient, predicted.celltype.l2, group) %>%
  summarize(count = n()) %>%
  ungroup() %>%
  group_by(patient) %>%
  mutate(normalized_count = count / sum(count)) %>%
  # filter for just subtypes of interest, add Trms
  filter(predicted.celltype.l2 %in% c("CD4 Proliferating", "CD8 Naive", "CD4 TEM", "CD8 TCM", "MAIT", "CD8 Trm", "CD4 Trm"),
         group != "no ICI")
...
# Calculate the Wilcoxon rank-sum test statistic and p-value
print(wilcox.test(test_data$normalized_count ~ test_data$group))
