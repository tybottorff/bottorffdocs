...
# Summarize degree of cell type junction crossover sharing for all cell types to make heatmap (TRA)
...
Heatmap(heatmap_matrix, 
        ...
        heatmap_legend_param = list(title = "TRA CDR3 AA\nlinkage count (log10)"),
        na_col = "white"
)

```

# Summarize degree of cell type junction crossover sharing for all cell types to make heatmap (TRB)
...
Heatmap(heatmap_matrix, 
        cluster_rows = FALSE,
        cluster_columns = FALSE,
        heatmap_legend_param = list(title = "TRB CDR3 AA\nlinkage count (log10)"),
        na_col = "white"
)
...
upset(as.data.frame(alpha_upset), sets = setdiff(colnames(alpha_upset), "cdr3"), sets.bar.color = "#56B4E9")

upset(as.data.frame(beta_upset), sets = setdiff(colnames(beta_upset), "cdr3"), sets.bar.color = "#56B4E9")
...
# looks like there's 6 CDR3s shared between Treg, CD8 Naive and CD8 TCM
# 10 CDR3s shared between CD8 TCM/Naive
# 323? between CD4 TCM CD8 TEM

test <- tcrs %>%
  filter(Chain == "TRA") %>%
  filter(cdr3 == "CAVRDSNYQLIW")
