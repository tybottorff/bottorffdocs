...
upset(as.data.frame(alpha_upset), sets = setdiff(colnames(alpha_upset), "cdr3"), sets.bar.color = "#56B4E9", sets.x.label = "TRA CDR3AAs per cell type", mainbar.y.label = "TRA CDR3AA linkages (log10) within cell type (combos)", point.size = 1.5, show.numbers = FALSE, scale.intersections = "log10", text.scale = 0.75)

upset(as.data.frame(beta_upset), sets = setdiff(colnames(alpha_upset), "cdr3"), sets.bar.color = "#56B4E9", sets.x.label = "TRB CDR3AAs per cell type", mainbar.y.label = "TRB CDR3AA linkages (log10) within cell type (combos)", point.size = 1.5, show.numbers = FALSE, scale.intersections = "log10", text.scale = 0.75)

```

# Investigate validity of upset plots

```{r}

#upset(as.data.frame(beta_upset), sets = setdiff(colnames(alpha_upset), "cdr3"))

# TRB
# 3 interactions between Tregs and CD8 naive NOT MATCHING 10 EXPECTED
# 11614 interactions within CD4 TCMs ok ~close enough to 12278 unique CDR3s in CD4 TCM cell group and makes sense to be lower since some CDR3s aren't shared even within a cell type ~MATCHES
# 4 interactions among all but CD4 CTL/Naive NOT MATCHING 0 EXPECTED

#unique(test$predicted.celltype.l2)

beta_upset %>%
  filter(Treg == 1 & `CD8 Naive` == 1 & `CD4 Proliferating` == 1 & `CD4 TCM` == 1) %>%
  print()

# 2 among Treg CD8 Naive CD4 Prolif & CD4 TCM I don't think on upset plot...

#colnames(beta_upset)

# 10 CDR3s seem to be present in both Treg and CD8 Naive (10) BUT upset plot only has number 3 for this...
