...
cd8_tem_trb_pgen <- prepped_data %>%
  filter(predicted.celltype.l2 == "CD8 TEM" & Chain == "TRB" & chunk == "25")

cd8_tem_trb_pgen

ggplot(cd8_tem_trb_pgen, aes(x = group, y = log10(pgen))) +
  geom_boxplot() +
  labs(title = "Top 25% expanded CD8 TEM TRBs")

cd8_trm_tra <- prepped_data %>%
  filter(predicted.celltype.l2 == "CD8 Trm" & Chain == "TRA" & chunk == "10") %>%
  mutate(pgen = log10(pgen)) %>%
  pivot_longer(cols = c("pgen", "cdr3_length"), names_to = "var") %>%
  mutate(var = case_when(
    var == "cdr3_length" ~ "CDR3 length (AAs)",
    var == "pgen" ~ "pgen (log10)"
  ))

cd8_trm_tra

ggplot(cd8_trm_tra, aes(x = var, y = value, fill = group)) +
  geom_boxplot() +
  labs(x = "", y = "Value")
