...
#A1, B11, B3 only have 1 count max in downsampled df
...
# likely there are ties issues but whatever for now, considering that then raises issue of > 2 per patient (or > n/patient)

unique_full_igor_combined_cd8_tems

```

# Create df of just solid conclusions

```{r}

top_3_per_patient_full <- unique_full_igor_combined_cd8_tems %>%
  group_by(Patient_ID) %>%
  filter(row_number() <= 3) %>%
  mutate(clonotype_rank = "Top 3\nper patient")

top_4_per_patient_full <- unique_full_igor_combined_cd8_tems %>%
  group_by(Patient_ID) %>%
  filter(row_number() <= 4) %>%
  mutate(clonotype_rank = "Top 4\nper patient")

top_5_per_patient_full <- unique_full_igor_combined_cd8_tems %>%
  group_by(Patient_ID) %>%
  filter(row_number() <= 5) %>%
  mutate(clonotype_rank = "Top 5\nper patient")

unique_full_cd8_tems <- unique_full_igor_combined_cd8_tems %>%
  full_join(top_3_per_patient_full) %>%
  full_join(top_4_per_patient_full) %>%
  full_join(top_5_per_patient_full)
...
unique_full_cd8_tems$clonotype_rank <- factor(unique_full_cd8_tems$clonotype_rank, levels = c("Top 3\nper patient", "Top 4\nper patient", "Top 5\nper patient", "All"))
...
# adj: 0.019666 0.019666 0.046920 0.105100

p_values <- c(0.009639, 0.009833, 0.03519, 0.1051)
...
p <- ggplot(unique_full_cd8_tems, aes(x = clonotype_rank, y = cdr3_length, fill = irAE)) +
  geom_boxplot() +
