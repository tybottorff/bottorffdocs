...
dehashed_combined_tcr_seq_with_rna <- as_tibble(readRDS("../saved_data/final_seurat_T_cells.rds")@meta.data)

dehashed_combined_tcr_seq_with_rna$TCR_name <- as.integer(rownames(dehashed_combined_tcr_seq_with_rna))
...
stitchr_data <- dehashed_combined_tcr_seq_with_rna %>%
  ...
  select(TRAV, TRAJ, TRA_CDR3, TRBV, TRBJ, TRB_CDR3, TRAC, TRBC) %>%
...
stitchr_data$TCR_name <- rownames(stitchr_data)

stitchr_data <- stitchr_data[, c("TCR_name", setdiff(names(stitchr_data), "TCR_name"))]
...
fasta_prep <- stitchr_output %>%
  left_join(dehashed_combined_tcr_seq_with_rna, by = "TCR_name")

fasta_df_alpha <- fasta_prep %>%
  filter(!(is.na(TRA_nt)),
         chain_TRA == "TRA") %>%
  mutate(TCR_name = paste(TCR_name, Patient_ID, sep = "_")) %>%
  ungroup()
...
imgt_df_1_alpha
# 5 productive
# 1724 rearranged but no junction found
# 3947 + 3368 (stop codons) unproductive
...
imgt_df_1_beta
# 3235 productive
# 13619 rearranged but no junction found
# 2394 + 1297 (stop codons) unproductive
