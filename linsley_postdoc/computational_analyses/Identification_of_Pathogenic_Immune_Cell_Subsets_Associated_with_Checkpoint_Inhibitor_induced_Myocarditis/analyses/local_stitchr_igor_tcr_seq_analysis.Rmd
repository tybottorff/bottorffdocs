...
test <- stitchr_output %>%
  select(TCR_name, Warnings.Errors, TRA_nt, TRB_nt, TRA_aa, TRB_aa, Linked_nt, Linked_aa)
  #filter(!(is.na(TRB_nt))) %>%
  #mutate(leader_error = if_else(str_detect(Warnings.Errors, "User specified leader sequence is not evenly divisible by 3"), "Yes", "No")) %>%
  #group_by(leader_error) %>%
  #summarize(count = n())
