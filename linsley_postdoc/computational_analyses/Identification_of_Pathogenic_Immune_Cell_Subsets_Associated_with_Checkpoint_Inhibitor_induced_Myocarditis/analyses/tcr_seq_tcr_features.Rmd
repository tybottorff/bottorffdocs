...
# Read in cell type assigned RDS

```{r}

immune.seurat <- readRDS("../saved_data/final_seurat.rds")

```

# Prep df to stratify/facet by cell type

```{r}

rna_seq_data <- as_tibble(immune.seurat@meta.data) %>%
  rename(Patient_ID = orig.ident)

rna_seq_data$barcode <- colnames(immune.seurat)

feature_df_with_cell_type <- inner_join(rna_seq_data, dehashed_combined_tcr_seq, by = c("barcode", "Patient_ID")) %>%
  #filter(predicted.celltype.l1 %in% c("CD4 T", "CD8 T", "other T"),
  #       predicted.celltype.l2 %in% c("CD4 CTL", "CD4 Naive", "CD4 Proliferating", "CD4 TCM", "CD4 TEM", "CD8 Naive", "CD8 Proliferating", "CD8 TCM", "CD8 TEM", "Treg")) %>%
  select(barcode, Patient_ID, irAE, chain, predicted.celltype.l1, predicted.celltype.l2, cdr3, cdr3_nt, predicted.celltype.l1.score, predicted.celltype.l2.score) %>%
  rowwise() %>%
  mutate(cdr3_length = nchar(cdr3),
         cdr3_hydrophobicity = calculate_hydrophobicity(cdr3, hydrophobicity_df)) %>%
  ungroup()

write.csv(feature_df_with_cell_type, "../saved_data/rna_seq_and_tcr_seq.csv")

```

# Plot feature analysis by cell type

```{r}

custom_labeller <- as_labeller(c("cdr3_length" = "CDR3\nAA length", "cdr3_hydrophobicity" = "CDR3\nhydrophobicity"))

plot_df <- feature_df_with_cell_type %>%
  select(-cdr3_nt) %>%
  pivot_longer(cols = starts_with("cdr3_"), values_to = "value") %>%
  filter(chain != "Multi")
...
ggplot(plot_df, aes(x = chain, y = value, fill = irAE)) +
  geom_boxplot() +
  facet_wrap(~ name + predicted.celltype.l1, scales = "free_y") +
  labs(x = "Chain", fill = "irAE", y = "Value")
