...
dehashed_combined_tcr_seq <- dehashed_combined_tcr_seq %>%
  ...
  # condense early/late chronology patient into 1 ID, mark Early vs. Late chronology for all Myocarditis patients
  mutate(time_point = ifelse(grepl("[EL]", Patient_ID), sub(".*([EL]).*", "\\1", Patient_ID), NA),
         Patient_ID = gsub("[EL]", "", Patient_ID)) %>%
...
# Extract IMGT/HighV-Quest output 3: CDR1/2s

```{r}

imgt_df_3_alpha <- read.csv("../saved_data/myocarditis_fixed_tra/3_Nt-sequences.csv") %>%
  filter(!is.na(CDR1.IMGT) & CDR1.IMGT != "",
         !is.na(CDR2.IMGT) & CDR2.IMGT != "") %>%
  mutate(cdr1 = as.character(translate(DNAStringSet(CDR1.IMGT))),
         cdr2 = as.character(translate(DNAStringSet(CDR2.IMGT)))) %>%
  select(Sequence.ID, cdr1, cdr2) %>%
  mutate(Chain = "TRA")
...
imgt_df_3 <- read.csv("../saved_data/myocarditis_fixed_trb/3_Nt-sequences.csv") %>%
  ...
  select(Sequence.ID, cdr1, cdr2) %>%
  mutate(Chain = "TRB") %>%
  full_join(imgt_df_3_alpha, by = c("Sequence.ID", "Chain", "cdr1", "cdr2"))

imgt_df_3

write.csv(imgt_df_3, "../saved_data/imgt_cdr1_2.csv")
