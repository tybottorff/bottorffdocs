...
ggplot(plot_df, aes(x = reorder(Epitope.species, -count), y = log10(count))) +
...
plot_df <- tcr_seq %>%
  ...
  mutate(patient_group_size = if_else(irAE == "Yes", 17, 8)) %>%
  group_by(Epitope.species, Epitope.gene, irAE, Patient_ID, orig.ident) %>%
  summarize(count = n() / patient_group_size) %>%
  arrange(-count) %>%
  print()

ggplot(plot_df %>% filter(Epitope.gene %in% c("BST2", "MLANA")), aes(x = irAE, y = count, fill = irAE)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ Epitope.gene) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1, size = 8)) +
  labs(x = "irAE development", y = "Sum of TCR counts\nper patient")
         
ggplot(plot_df %>% filter(Epitope.gene %in% c("BST2", "MLANA")), aes(x = Patient_ID, y = count, fill = irAE)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ Epitope.gene) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1, size = 8)) +
  labs(x = "Patient ID", y = "TCR count")
