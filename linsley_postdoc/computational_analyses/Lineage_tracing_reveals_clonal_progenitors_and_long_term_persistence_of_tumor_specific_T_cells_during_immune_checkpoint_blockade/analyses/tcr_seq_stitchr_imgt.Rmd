...
tcr_files <- grep("TCR_filtered_contig_annotations", files, value = TRUE)
...
merged_df <- do.call(rbind, dfs) %>%
  filter(high_confidence == "True",
         full_length == "True",
         productive == "True") %>%
  mutate(barcode = paste(barcode, gsub(".$", "", str_extract(file_path, "(?<=MSK).*?(?=TCR)")), sep = "_"))
...
imgt_df_1_alpha <- read.csv("../saved_data/lineage_tracing_tra/1_Summary.csv") %>%
  group_by(V.DOMAIN.Functionality) %>%
  summarize(count = n())

imgt_df_1_alpha
# 11 productive
# 10,836 rearranged but no junction found
# 6,942 + 9,726 (stop codons) unproductive

imgt_df_1_beta <- read.csv("../saved_data/lineage_tracing_trb/1_Summary.csv") %>%
  group_by(V.DOMAIN.Functionality) %>%
  summarize(count = n())

imgt_df_1_beta
# 9824 productive
# 72,184 rearranged but no junction found
# 6,329 + 6,899 (stop codons) unproductive
