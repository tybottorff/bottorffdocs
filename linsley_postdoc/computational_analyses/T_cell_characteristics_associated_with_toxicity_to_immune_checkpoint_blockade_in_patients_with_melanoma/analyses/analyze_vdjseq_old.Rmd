...
stitchr_output_analysis <- read_tsv("../saved_data/vdj_stitchr_out.tsv")

error_rows <- grep("Cannot stitch", stitchr_output_analysis$`Warnings/Errors`)

stitchr_output_analysis[error_rows, ]
...
fasta_df <- read_tsv("../saved_data/vdj_stitchr_out.tsv") %>%
  filter(!(is.na(Linked_nt))) %>%
