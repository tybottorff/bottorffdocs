...
stitchr_df <- combined_data %>%
  ...
  mutate(TRA_leader	= NA,
         ...
         TRB_5_prime_seq = NA,
         TRB_3_prime_seq = NA) %>%
  mutate(across(all_of(c("TRAV", "TRAJ", "TRBV", "TRBJ", "TRAC", "TRBC")), ~ifelse(!stringr::str_detect(., "\\*01$"), paste0(., "*01"), .)))
...
# Investigate stitchr errors

```{r}

stitchr_errors <- read_tsv("../saved_data/stitchr_out.tsv") %>%
  mutate(alpha_fail = is.na(TRA_nt),
         beta_fail = is.na(TRB_nt)) %>%
  select(TCR_name, TRA_nt, beta_fail, alpha_fail, TRA_CDR3, TRAV) %>%
  filter(beta_fail == FALSE)

stitchr_errors

# so it does look like failed alpha stitching TRAVs don't have allele specified, and successful ones do (same for beta...) BUT I thought I tried stitching with *01 allele specified for all and didn't recover any more barcodes...

# worth retrying thimble/stitchr with *01 and rechecking errors...
...
stitchr_output_alpha_df <- read_tsv("../saved_data/stitchr_out.tsv") %>%
  filter(!(is.na(TRA_nt)))
...
stitchr_output_beta_df <- read_tsv("../saved_data/stitchr_out.tsv") %>%
  filter(!(is.na(TRB_nt)))
...
fasta_df_alpha <- read_tsv("../saved_data/stitchr_out.tsv") %>%
  filter(!(is.na(TRA_nt)),
         Chain == "TRA") %>%
  rename(Barcode = TCR_name) %>%
  left_join(combined_data, by = "Barcode") %>%
  mutate(Barcode = paste(Barcode, suffix_mapping$suffix[suffix_mapping$patient.ID == patient.ID], sep = "")) %>%
  ungroup()
...
fasta_df_beta <- read_tsv("../saved_data/stitchr_out.tsv") %>%
  filter(!(is.na(TRB_nt)),
         Chain == "TRB") %>%
  rename(Barcode = TCR_name) %>%
  left_join(combined_data, by = "Barcode") %>%
...
imgt_df_8_alpha <- read.table("../saved_data/IMGT_HIGHV_QUEST/alpha/8_V-REGION-nt-mutation-statistics.txt", header = TRUE, sep = "\t") %>%
...
imgt_df_8_beta <- read.table("../saved_data/IMGT_HIGHV_QUEST/beta/8_V-REGION-nt-mutation-statistics.txt", header = TRUE, sep = "\t") %>%
  select(-Sequence.number) %>%
  rename(Barcode = Sequence.ID)
...
# join with patient ID to get severe_irAE yes/no --> plots
# join with cell_type for barcodes with suffixes (i.e. scRNAseq paired barcodes) --> plots (expecting I lost too many to make useful UMAP overlays)

imgt_df_8

```

# Analyze IMGT/HighV-Quest output 6: junction data

```{r}

#imgt_df_6_alpha <- read.table("../saved_data/IMGT_HIGHV_QUEST/alpha/6_Junction.txt", header = TRUE, sep = "\t")
